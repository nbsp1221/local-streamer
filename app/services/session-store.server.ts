...
export async function cleanupExpiredSessions(): Promise<void> {
  ...
  const now = new Date();
  const validSessions = sessions.filter(session => session.expiresAt > now && session.isActive);
...
export async function getSessionsByUserId(userId: string): Promise<Session[]> {
  ...
  return sessions.filter(session => session.userId === userId && session.expiresAt > now);
}
