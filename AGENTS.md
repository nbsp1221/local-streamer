...
## Testing Guidelines
Vitest drives unit and integration coverage. Name files `*.test.ts`, keep arrange/act/assert sections clear, and maintain ≥75% coverage with `bun run test:run -- --coverage` as noted in `CLAUDE.md`. Use `bun run test:run`, `bun run test:modules`, `bun run test:integration`, and `bun run test:ui-dom` for focused Vitest work; use `bun run vitest:ui` only as a developer UI launcher, not as verification. Prefer `jsdom + React Testing Library + jest-dom + user-event` for new component-level UI tests instead of string-based markup assertions. Use `bun run test` before handoff so the Bun smoke layers run too. For end-to-end work, follow `docs/E2E_TESTING_GUIDE.md`: start the dev server, cover API flows with cURL, and escalate to Playwright for UI, playback, or DRM checks.
...
4. `docs/research/*` for topic-specific investigations such as playback or competitor analysis.
5. `docs/plans/*` for temporary execution plans. Treat them as working notes, not the source of truth for current repo state.
